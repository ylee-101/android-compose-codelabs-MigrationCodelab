...
private fun PlantDetailContent(plant : Plant) {
    Surface {
        Column(Modifier.padding(dimensionResource(id = R.dimen.margin_normal))) {
            PlantDetailName(name = plant.name)
...
@Composable
private fun PlantDetailName(name : String) {
    Text(
        text = name,
...
private fun PlantDetailPreview() {
    val plant = Plant(
        plantId = "id", name = "Apple",
        description = "HTML<br><br>description<br>APPLE",
        growZoneNumber = 7, wateringInterval = 30
    )
    MdcTheme{
...
@Composable
private fun PlantDetailDarkPreview() {
    val plant = Plant(
        plantId = "id", name = "Apple",
        description = "HTML<br><br>description<br>APPLE",
        growZoneNumber = 7, wateringInterval = 30
...
fun PlantWatering(wateringInterval : Int) {
    Column(Modifier.fillMaxWidth()) {
        ...
        Text(
            ...
            modifier = defaultModifier.padding(top = normalPadding)
        )
...
private fun PlantWateringPreview() {
    MdcTheme{
        PlantWatering(wateringInterval = 20)
    }
}
